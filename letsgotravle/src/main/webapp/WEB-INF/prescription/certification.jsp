...
<div class="container">
    <div class="logo">
        <img src="src/main/resources/static/images/image71.png" alt="LETS GO TRAVLE 로고">
    </div>
...
$('#certification-form').on('submit', function(e) {
    ...
    $.ajax({
        url: '${pageContext.request.contextPath}/prescription/processCertification.do', // 서버 엔드포인트
        method: 'POST',
        data: formData,
        success: function(response) {
            ...
            if (response.redirectToSecureInput) {
                ...
                window.location.href = '${pageContext.request.contextPath}/prescription/additionalCertification.do';
            }
        },
...
$('#refresh-secureNo').on('click', function () {
    $.ajax({
        url: '${pageContext.request.contextPath}/prescription/refreshSecureNo.do', // 새 보안문자 요청 엔드포인트
  ...
  $('#secure-submit').on('click', function () {
    ...
    $.ajax({
        url: '${pageContext.request.contextPath}/prescription/processSecureInput.do', // 추가 인증 엔드포인트
        method: 'POST',
        data: secureData,
        success: function (response) {
            if (response.smsSent) {
                // SMS 인증번호 입력 페이지로 이동
                window.location.href = '${pageContext.request.contextPath}/prescription/smsAuthenticationPage.do';
            } else {
