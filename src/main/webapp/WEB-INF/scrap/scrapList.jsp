...
<!-- 로그인 여부 확인: 세션에서 midx가 없으면 로그인 페이지로 리디렉트 -->
...
<div class="pagination">
    ...
    <c:if test="${pageMaker.prev}">
        <a href="${pageContext.request.contextPath}/scrap/scrapList.do?midx=${sessionScope.midx}&page=${pageMaker.startPage - 1}">◀</a>
    </c:if>

    <!-- 페이지 번호 목록 -->
    <c:forEach var="i" begin="${pageMaker.startPage}" end="${pageMaker.endPage}">
        <c:set var="currentPage" value="${not empty param.page ? param.page : 1}" />
        <a href="${pageContext.request.contextPath}/scrap/scrapList.do?midx=${sessionScope.midx}&page=${i}"
           class="${i == currentPage ? 'active' : ''}">${i}</a>
    </c:forEach>

    <!-- 다음 페이지 버튼 -->
    <c:if test="${pageMaker.next}">
        <a href="${pageContext.request.contextPath}/scrap/scrapList.do?midx=${sessionScope.midx}&page=${pageMaker.endPage + 1}">▶</a>
    </c:if>
</div>
